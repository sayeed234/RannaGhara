...
<?php
    ...
    $cart_product_id = $_GET['cart_brk'];
    header("Location: /rannaghara/breakfast.php");

    if ($con) {
        $check_pro_query = "SELECT * FROM breakfastnr WHERE product_id LIKE '%$cart_product_id%'";
        $result_query = mysqli_query($con, $check_pro_query);
        $count = mysqli_num_rows($result_query);
        
        if($count == 0){
            
            $check_cart_product_id = "SELECT * FROM breakfastnr WHERE post_idnr = '$cart_product_id'";
